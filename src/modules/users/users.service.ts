...
import { generateSecurePassword } from '../../utils/password.util.js';
import { generateOTP, storeVerificationOTP } from '../../utils/otp.util.js';
...
export class UsersService {
  ...
  async create(input: any, creatorOrgId?: string | null): Promise<UserResponse> {
    ...
    // Determine if this is an invitation (autogenerated password)
    const isInvite = !input.password;
    const rawPassword = input.password || generateSecurePassword();
    const passwordHash = await bcrypt.hash(rawPassword, env.BCRYPT_ROUNDS);
    ...
    const user = await prisma.user.create({
      data: {
        ...
        isEmailVerified: false, // Must verify via invite/onboarding link
      },
      select: this.userSelect,
    });

    if (isInvite) {
      // Generate and store verification OTP for the direct link
      const otp = generateOTP();
      await storeVerificationOTP(user.email, otp);
      
      // Send the invitation email with the temporary password and verification link
      sendInviteEmail(user.email, rawPassword, otp).catch((err) => {
        logger.error('Failed to send onboarding invitation email', { error: err, email: user.email });
      });
      
      logger.info('User invited with autogenerated password', { userId: user.id, createdBy: 'admin' });
    } else {
      logger.info('User created with manual password', { userId: user.id, createdBy: 'admin' });
    }
